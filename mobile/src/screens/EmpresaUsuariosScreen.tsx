...
export default function EmpresaUsuariosScreen({ route, navigation }: any) {
  ...
  useEffect(() => {
    navigation.setOptions({ title: `Usuários — ${empresaNome}` });
    carregar();
  }, []);

  async function carregar() {
    try { setUsuarios(await EmpresasAPI.listarUsuarios(empresaId)); }
  ...
  async function criar() {
    ...
    setSalvando(true);
    try {
      await EmpresasAPI.criarUsuario(empresaId, { nome, email, senha, role: 'company_admin' });
      setModalAberto(false);
      setNome(''); setEmail(''); setSenha('');
      carregar();
    } catch (err) {
      const msg = err instanceof ApiError ? (err.data?.erro || 'Erro ao criar usuário.') : 'Erro de conexão.';
  ...
  function excluir(usuario: any) {
    Alert.alert(
      'Excluir usuário', `Remover "${usuario.nome}" desta empresa?`,
      [{ text: 'Cancelar', style: 'cancel' }, {
        text: 'Excluir', style: 'destructive',
        onPress: async () => {
          try { await EmpresasAPI.excluirUsuario(empresaId, usuario.id); carregar(); }
  ...
  return (
    <>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.empty}>Nenhum usuário nesta empresa.</Text>}
        renderItem={({ item }) => (
...
const styles = StyleSheet.create({
  ...
  btnNovo: {
    position: 'absolute', bottom: 16, left: 16, right: 16, backgroundColor: '#3b82f6', borderRadius: 10,
  ...
  btnCriar: { flex: 1, backgroundColor: '#3b82f6', borderRadius: 10, paddingVertical: 12, alignItems: 'center' },
